...
pub struct DcamCamera{
    ...
    image_buffer: Box<DcamImage>
}

pub struct DcamImage{
    metadata : DCAMBUF_FRAME,
    data : [u8]
}

impl CameraModel::Frame for DcamImage{
    fn format(&self) -> CameraModel::PixelFormat{
        use std::convert::TryInto;
        match self.metadata.type_.try_into() {
            Ok(dcam::PixelType::Mono16) => CameraModel::PixelFormat::MonoU16,
            Ok(dcam::PixelType::Mono8) => CameraModel::PixelFormat::MonoU8,
            Ok(dcam::PixelType::None) => unreachable!(),
            //TODO(hayley): handle this more gracefully
            Err(e) => unreachable!()
        }
    }
	...
	fn pixels<'a>(&'a self) -> CameraModel::Pixels<'a> {
		match self.format() {
			CameraModel::PixelFormat::MonoU8 => CameraModel::Pixels::MonoU8(&self.data),
			CameraModel::PixelFormat::MonoU16 => CameraModel::Pixels::MonoU16(unsafe{
...
impl CameraModel::Camera for DcamCamera{
    type Frame = DcamImage;
    fn new() -> Self{
		let dev_open = unsafe{
        ...
        let mut ret = DcamCamera{
            ...
            image_buffer : {
                ...
                unsafe {
                    ...
                    unsafe{Box::from_raw(std::mem::transmute::<&mut [u8], &mut DcamImage>(Box::leak(values)))}
                }
            }
        };
		unsafe{
		let attach = dcam4_sys::DCAMBUF_ATTACH{
			...
			buffer : &mut (std::mem::transmute::<&mut DcamImage, &mut [u8]>(&mut ret.image_buffer).as_mut_ptr_range().start as *mut _),
			buffercount : 1
		};
